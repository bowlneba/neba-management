...
- Include `description` (single sentence, actionable outcome), `agent` (explicitly choose `agent` when the prompt runs as an agent), and `tools` (minimal set of tool bundles required to fulfill the prompt).
